...
export class PostService extends BaseService {
    ...
    async createPost(data) {
        ...
        if (data.categories?.length) {
            await CategoryModel.updateMany({ _id: { $in: data.categories } }, { $inc: { postCount: 1 } });
        }
        if (data.tags?.length) {
            await TagModel.updateMany({ _id: { $in: data.tags } }, { $inc: { postCount: 1 } });
        }
        // Clear cache
        await this.cache.deleteByPattern('posts:*');
        return post;
    }
    async getPostBySlug(slug, includePrivate = false) {
    ...
    async getPublishedPosts(pagination, filters) {
    ...
    async updatePost(id, data) {
        ...
        if (data.categories !== undefined) {
            ...
            if (removedCategories.length) {
                await CategoryModel.updateMany({ _id: { $in: removedCategories } }, { $inc: { postCount: -1 } });
            }
            if (addedCategories.length) {
                await CategoryModel.updateMany({ _id: { $in: addedCategories } }, { $inc: { postCount: 1 } });
            }
        }
        // Handle tag changes
        if (data.tags !== undefined) {
            ...
            if (removedTags.length) {
                await TagModel.updateMany({ _id: { $in: removedTags } }, { $inc: { postCount: -1 } });
            }
            if (addedTags.length) {
                await TagModel.updateMany({ _id: { $in: addedTags } }, { $inc: { postCount: 1 } });
            }
        }
    ...
    async deletePost(id) {
        ...
        if (post.categories?.length) {
            await CategoryModel.updateMany({ _id: { $in: post.categories } }, { $inc: { postCount: -1 } });
        }
        if (post.tags?.length) {
            await TagModel.updateMany({ _id: { $in: post.tags } }, { $inc: { postCount: -1 } });
        }
    ...
    async getPostsByStatus(status, pagination) {
